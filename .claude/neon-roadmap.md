...
- 🚧 **Greenlet half REMAINING (next item):** the **trading** intent still hits `greenlet_spawn has not been called` (async/sync) in `_generate_trading_response` → `ai_trading_service` recommendation path (sub-generators have defensive try/excepts, so it needs a live stack trace to pin — likely a lazy-relationship access on the async session). Non-trading intents are unaffected. Also: combat/colony/station/strategic still return "coming soon" content stubs (separate content gap, Max-call on AI dialogue).
