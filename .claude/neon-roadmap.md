...
- 🟢 **client apiRequest reads data.detail but server error envelope sends data.message** (services/api.ts:30) — so HTTPException messages render as generic "API Error: <code>" in apiRequest-based UIs (e.g. combat). Fix: `data.detail || data.message`. eff1, broad UX win, low-risk additive. NEW (found during the port fix).
