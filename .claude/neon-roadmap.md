...
- 🚧 **Remaining ARIA items (separate):** trade *numbers* are fabricated (`ai_trading_service._get_price_history`→[], profit→1500, sectors→id+1/2/3 — de-mock with real market/adjacency data); consider `expire_on_commit=False` on `AsyncSessionLocal` as the systemic fix for this greenlet class (broader blast radius — wants adversarial review); combat/colony/station/strategic content depth is a Max-call on AI dialogue.
