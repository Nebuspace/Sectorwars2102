...
- 🚧 **Remaining ARIA flavor stubs:** `_identify_price_factors` (static ["Market demand","Supply levels","Player activity"]) + `_assess_sector_risk` (0.3) still fabricated, emitted alongside now-real price data; combat/colony/station/strategic content depth is a Max-call on AI dialogue. `expire_on_commit=False` on AsyncSessionLocal would systemically kill the greenlet class (wants adversarial review).
