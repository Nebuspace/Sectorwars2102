...
| 7 | **Dead code purge + typecheck gate** | P5.* purge + tsc gate in CI | ✅ DONE — purge (runs 6/10/12, ~8,500+ lines); **tsc gate WIRED run 11 (`34f3fe2`)**: `build` = `tsc --noEmit && vite build`, enforced by the Dockerfile production stage (`npm run build`); admin-ui tsc errors burned **28 → 0** |
...
- 🟡 **/drones/deploy random deploymentId FIXED build-verified (2026-06-14, `d7a2cf6`)** — now returns the real DroneDeployment row id(s) instead of a random uuid4 (recall did `db.get(DroneDeployment, id)` → 404 on the fake id). Correct by inspection (deploy_drone refreshes the row, so `.id` is safe). **FULL-FLOW PROOF BLOCKED — two deeper pre-existing bugs in this unmounted/Max-gated subsystem surfaced and were NOT fixed (out of scope, design/Max-gated):** (1) `DroneStatus` enum has **no idle/available state** (only deployed/combat/returning/destroyed/damaged) so `create_drone` drones default to `deployed` and are never deployable; (2) `DroneService.deploy_drone` raises `greenlet_spawn ... await_only` (ASYNC-SYNC mismatch) → POST /drones/deploy 500s. Both need a dedicated drone-subsystem repair pass (also gates the drone create/upgrade/repair costs already flagged Max-gated). The contract id fix is correct and will work once the subsystem deploys at all.
