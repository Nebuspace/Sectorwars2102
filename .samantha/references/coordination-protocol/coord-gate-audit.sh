...
#
# READY-vs-gated cross-check: independently of the 3 mechanical gate kinds
# above, also flags a row whose Status column says READY while its own
# `gated` column already says yes/design-ruling/human-gated — a hand-
# maintained-field drift bug (2 incidents same session, see coord-dir),
# not the same failure mode as the mechanical audit missing a marker.
# A mismatch prints "⚠️ GATE-MISMATCH" per row and exits 3 (see EXIT CODES).
#
...
#
# EXIT CODES: 0 = successful run, no READY-vs-gated mismatch. 2 = usage/IO
# error. 3 = successful run, but >=1 row has status=READY contradicted by its
# own gated column (see READY-vs-gated cross-check above) — the audit report
# still printed in full above the FAIL line.
set -euo pipefail
...
GATED_TRUE_PATTERN="${COORD_GATE_GATED_TRUE_PATTERN:-(?i)\\b(?:yes|design[- ]ruling|human-gated)\\b}"
...
Usage: coord-gate-audit.sh --queue <queue-*.md> [--wo WO-ID] [--code-root PATH] [--docs-root PATH]
  ...
  --gated-true-pattern  Regex for the READY-vs-gated cross-check's "this
                        gated-column value counts as gated" test
                        (default: \b(?:yes|design[- ]ruling|human-gated)\b)
EOF
  exit 2
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --gated-true-pattern) GATED_TRUE_PATTERN="${2:-}"; shift 2 ;;
...
export COORD_GATE_GATED_TRUE_PATTERN="$GATED_TRUE_PATTERN"
...
# READY-vs-gated cross-check (WO-INFRA-GATE-CROSS-CHECK): a row's own `gated`
# column can drift from its `Status` column across many discovery-pass edits
# (origin: 2 incidents same session — the ADR-0038-0092 mis-stage, and 7 rows
# staged status=READY while their own gated column already said yes/design-
# ruling). This catches that class mechanically instead of relying on someone
# reading both columns together.
STATUS_READY = re.compile(r"(?i)^\s*READY\b")
GATED_TRUE = re.compile(os.environ["COORD_GATE_GATED_TRUE_PATTERN"])
...
def main() -> int:
    ...
    for row in rows:
        ...
        # READY-vs-gated cross-check: the row's own `gated` column is a
        # separate, hand-maintained signal from the mechanical audit above —
        # a discovery pass can stage status=READY while gated already says
        # yes/design-ruling/human-gated. Flag the contradiction; this is
        # never blank-triggered (an empty gated column is UNVERIFIED-FOR-
        # GATING per queue-hygiene convention, not a confirmed mismatch).
        gated_col = (row.get("gated") or row.get("Gated") or "").strip()
        if gated_col and STATUS_READY.search(status) and GATED_TRUE.search(gated_col):
            mismatched += 1
            print(f"  ⚠️ GATE-MISMATCH: status=READY but gated column says: {gated_col!r}")
        print()
    ...
    print(f"SUMMARY: audited={audited} flagged={flagged} clean={audited - flagged} gate-mismatched={mismatched}")
    ...
    if mismatched:
        print(f"FAIL: {mismatched} row(s) with status=READY contradicted by their own gated column.")
        return 3
    return 0
