...
describe('AdvancedAnalytics generate/export (LEG-143)', () => {
  beforeEach(() => {
  ...
  it('posts generate via shared api and shows the returned report', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/api/v1/admin/reports/generate', expect.objectContaining({
      name: 'Ops Snapshot',
      metrics: ['player_total_count'],
    }));
    expect(screen.getByText('Ops Snapshot')).toBeTruthy();
    expect(screen.getByTestId('analytics-save-message').textContent).toMatch(/generated successfully/i);
