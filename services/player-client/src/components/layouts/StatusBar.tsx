...
const StatusBar: React.FC = () => {
  ...
  // Reuses rankingAPI.getRank() + RankDisplay's TIER_COLORS / rank-badge art.
  const [rankLevel, setRankLevel] = useState<number | null>(null);
  const [rankTier, setRankTier] = useState<string | null>(null);
  const [rankUnavailable, setRankUnavailable] = useState(false);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setRankTier(typeof data.rank_tier === 'string' ? data.rank_tier : null);
        setRankUnavailable(false);
      } catch {
        if (cancelled) return;
        setRankLevel(null);
        setRankTier(null);
        setRankUnavailable(true);
      }
    })();
    return () => {
      cancelled = true;
  ...
  const rankTierColor = rankTier ? TIER_COLORS[rankTier] || '#888888' : '#888888';
  const rankBadgeLabel =
    rankUnavailable || rankLevel === null ? '—' : String(rankLevel);
  ...
  return (
    <div className="sbar status-bar">
      ...
      <div className="sb-vitals">
        ...
        <span
          className="vit sb-stat rank-insignia"
          title={rankTier ? `Military rank tier: ${rankTier}` : 'Military rank'}
          aria-label={
            rankUnavailable
              ? 'Military rank unavailable'
              : `Military rank level ${rankBadgeLabel}`
          }
        >
          <span className="rank-badge" style={{ borderColor: rankTierColor }}>
            <span className="rank-level">{rankBadgeLabel}</span>
          </span>
        </span>
        <span
          className="vit repb"
