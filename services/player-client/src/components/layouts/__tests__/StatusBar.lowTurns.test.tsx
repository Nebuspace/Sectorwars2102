...
/**
 * StatusBar — low-turn scarcity warning (WO-PROG-TURN-VISIBILITY).
 *
 ...
 * not read as "low").
 *
 * RE-POINTED from PlayerVitalsHud.lowTurns.test.tsx (WO-CLEANUP-
 * PLAYERVITALSHUD) — PlayerVitalsHud is deleted; StatusBar now owns this
 * behavior (StatusBar.tsx's `lowTurns`/`sb-turns-low`, carried over
 * verbatim from PlayerVitalsHud's own computation per StatusBar.tsx's own
 * doc-comment). Mirrors Dashboard.icons.test.tsx's seam: jsdom +
 * react-dom/client createRoot + act(), no RTL. LogoutButton (react-router's
 * useNavigate) is stubbed — it's shipped/out-of-scope chrome unrelated to
 * this assertion, same rationale StatusBar.smoke.test.tsx uses.
 */
...
describe('StatusBar — low-turn warning', () => {
  ...
  it('applies the low-turns class at 49 (below the <50 threshold)', async () => {
    ...
    expect(container.querySelector('.sb-turns-low')).not.toBeNull();
  });
  ...
  it('does not apply the low-turns class at exactly 50 (the boundary is not low)', async () => {
    ...
    expect(container.querySelector('.sb-turns-low')).toBeNull();
  });
