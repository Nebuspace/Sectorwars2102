...
/**
 * GameDashboard — asteroid HARVEST pre-click grey-out (LEG-2728 / mining.md:251).
 */
...
const mockGetYieldPreview = vi.fn();
...
vi.mock('../../../services/api', () => ({
  ...
  miningAPI: {
    getYieldPreview: (...a: unknown[]) => mockGetYieldPreview(...a),
    harvest: vi.fn(),
  },
}));
...
describe('GameDashboard — HARVEST pre-click gate (LEG-2728)', () => {
  let container: HTMLElement;
  let root: ReturnType<typeof createRoot>;
  let errorSpy: ReturnType<typeof vi.spyOn>;

  beforeEach(() => {
    mockGetYieldPreview.mockReset();
  ...
  it('disables HARVEST with gate copy in title when preview rejects no_mining_laser', async () => {
    mockGetYieldPreview.mockRejectedValue(new Error('no_mining_laser'));
    await mount();

    const btn = harvestBtn();
    expect(btn).toBeTruthy();
    expect(btn?.disabled).toBe(true);
    expect(btn?.title).toContain('No mining laser equipped');
    expect(btn?.getAttribute('aria-label')).toContain('No mining laser equipped');
  });

  it('leaves HARVEST enabled when preview succeeds', async () => {
    mockGetYieldPreview.mockResolvedValue({
      success: true,
      reason: null,
      ore_lo: 4,
      ore_hi: 8,
    ...
    expect(btn?.title).toContain('Deploy the mining laser');
  });
});
