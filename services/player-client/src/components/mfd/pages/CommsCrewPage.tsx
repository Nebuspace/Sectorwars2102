...
/** Canon copy when server rejects 51st send with thread_limit_exceeded (messaging.md § Threading). */
const THREAD_LIMIT_EXCEEDED_COPY =
  'THREAD LIMIT EXCEEDED — THREADS SUPPORT UP TO 50 MESSAGES (51ST SEND REJECTED)';
...
const CommsCrewPage: React.FC = () => {
  ...
  // LEG-2603 — tip conversations list (latest Message per thread).
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const rawDetail = error?.response?.data?.detail ?? error?.data?.detail;
      const message =
        rawDetail === 'thread_limit_exceeded'
          ? THREAD_LIMIT_EXCEEDED_COPY
