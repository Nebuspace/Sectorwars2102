...
/**
 * GameContext — citadelAPI.setAutoDeposit (WO-WIRE-CITADEL-AUTO-DEPOSIT-API).
 */
import React, { act } from 'react';
import { createRoot } from 'react-dom/client';
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';

const { mockGet, mockPost, mockSetAutoDeposit } = vi.hoisted(() => ({
  mockGet: vi.fn(),
  mockPost: vi.fn(),
  mockSetAutoDeposit: vi.fn(),
}));
...
vi.mock('../../services/api', async () => {
  ...
  return {
    ...
    citadelAPI: {
      ...actual.citadelAPI,
      setAutoDeposit: (...a: unknown[]) => mockSetAutoDeposit(...a),
    },
    sectorAPI: {
      ...actual.sectorAPI,
...
describe('GameContext citadelAPI.setAutoDeposit', () => {
  ...
  beforeEach(async () => {
    ...
    mockSetAutoDeposit.mockResolvedValue({ success: true, auto_deposit: true });
    captured = null;
  ...
  it('setCitadelAutoDeposit routes through citadelAPI.setAutoDeposit', async () => {
    let result: unknown;
    await act(async () => {
      result = await captured!.setCitadelAutoDeposit('planet-9', true);
      await flush();
    });
    expect(mockSetAutoDeposit).toHaveBeenCalledWith('planet-9', true);
    expect(result).toEqual({ success: true, auto_deposit: true });
    const rawPosts = mockPost.mock.calls.filter((c) =>
      String(c[0]).includes('/citadel/auto-deposit'),
    );
