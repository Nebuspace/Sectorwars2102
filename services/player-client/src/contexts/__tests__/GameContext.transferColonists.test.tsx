...
/**
 * GameContext — planetaryAPI.transferColonists
 * (WO-WIRE-PLANETARY-TRANSFER-COLONISTS).
 */
import React, { act } from 'react';
import { createRoot } from 'react-dom/client';
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';

const { mockGet, mockPost, mockTransferColonists } = vi.hoisted(() => ({
  mockGet: vi.fn(),
  mockPost: vi.fn(),
  mockTransferColonists: vi.fn(),
}));
...
vi.mock('../../services/api', async () => {
  ...
  return {
    ...
    planetaryAPI: {
      ...actual.planetaryAPI,
      transferColonists: (...a: unknown[]) => mockTransferColonists(...a),
      getPlanet: vi.fn(),
      getPlanets: vi.fn(),
    },
    sectorAPI: {
...
describe('GameContext planetaryAPI.transferColonists', () => {
  ...
  beforeEach(async () => {
    ...
    mockTransferColonists.mockResolvedValue({ success: true, transferred: 5 });
    captured = null;
  ...
  it('transferColonists routes through planetaryAPI.transferColonists', async () => {
    let result: unknown;
    await act(async () => {
      result = await captured!.transferColonists('planet-9', 'embark', 5);
      await flush();
    });
    expect(mockTransferColonists).toHaveBeenCalledWith('planet-9', 'embark', 5);
    expect(result).toEqual({ success: true, transferred: 5 });
    const rawPosts = mockPost.mock.calls.filter((c) =>
      String(c[0]).includes('/colonists/transfer'),
    );
