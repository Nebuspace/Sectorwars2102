...
/**
 * GameContext — planetaryAPI.allocateColonists (WO-WIRE-PLANETARY-ALLOCATE).
 */
import React, { act } from 'react';
import { createRoot } from 'react-dom/client';
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';

const { mockGet, mockPost, mockPut, mockAllocate } = vi.hoisted(() => ({
  mockGet: vi.fn(),
  mockPost: vi.fn(),
  mockPut: vi.fn(),
  mockAllocate: vi.fn(),
...
vi.mock('../../services/api', async () => {
  ...
  return {
    ...
    planetaryAPI: {
      ...actual.planetaryAPI,
      allocateColonists: (...a: unknown[]) => mockAllocate(...a),
    },
    sectorAPI: {
      ...actual.sectorAPI,
...
describe('GameContext planetaryAPI.allocateColonists', () => {
  ...
  beforeEach(async () => {
    ...
    mockAllocate.mockResolvedValue({
      success: true,
      allocations: { fuel: 10, organics: 5, equipment: 2, unused: 0 },
    });
    captured = null;
  ...
  it('updatePlanetAllocation routes through planetaryAPI.allocateColonists', async () => {
    const alloc = { fuel: 10, organics: 5, equipment: 2 };
    let result: unknown;
    await act(async () => {
      result = await captured!.updatePlanetAllocation('planet-9', alloc);
      await flush();
    });
    expect(mockAllocate).toHaveBeenCalledWith('planet-9', alloc);
    expect(result).toEqual({
      success: true,
      allocations: { fuel: 10, organics: 5, equipment: 2, unused: 0 },
    });
    const rawPuts = mockPut.mock.calls.filter((c) =>
      String(c[0]).includes('/allocate'),
    );
