...
/**
 * GameContext — planetaryAPI.land / leave (WO-WIRE-PLANETARY-LAND-LEAVE).
 */
import React, { act } from 'react';
import { createRoot } from 'react-dom/client';
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';

const { mockGet, mockPost, mockLand, mockLeave } = vi.hoisted(() => ({
  mockGet: vi.fn(),
  mockPost: vi.fn(),
  mockLand: vi.fn(),
  mockLeave: vi.fn(),
...
describe('GameContext planetaryAPI land/leave', () => {
  ...
  beforeEach(async () => {
    ...
    mockLand.mockResolvedValue({ success: true, is_landed: true });
    mockLeave.mockResolvedValue({ success: true, is_landed: false });
    captured = null;
  ...
  it('landOnPlanet routes through planetaryAPI.land', async () => {
    let result: unknown;
    await act(async () => {
      result = await captured!.landOnPlanet('planet-9');
      await flush();
    });
    expect(mockLand).toHaveBeenCalledWith('planet-9');
    expect(result).toEqual({ success: true, is_landed: true });
    const rawPosts = mockPost.mock.calls.filter((c) =>
      String(c[0]).includes('/planets/land'),
    );
    expect(rawPosts).toHaveLength(0);
  });

  it('leavePlanet routes through planetaryAPI.leave', async () => {
    let result: unknown;
    await act(async () => {
      result = await captured!.leavePlanet();
      await flush();
    });
    expect(mockLeave).toHaveBeenCalledWith();
    expect(result).toEqual({ success: true, is_landed: false });
    const rawPosts = mockPost.mock.calls.filter((c) =>
      String(c[0]).includes('/planets/leave'),
    );
    expect(rawPosts).toHaveLength(0);
  });
});
