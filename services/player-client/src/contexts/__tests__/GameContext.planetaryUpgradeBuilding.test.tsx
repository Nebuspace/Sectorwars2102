...
/**
 * GameContext — planetaryAPI.upgradeBuilding (WO-WIRE-PLANETARY-UPGRADE-BUILDING).
 */
import React, { act } from 'react';
import { createRoot } from 'react-dom/client';
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';

const { mockGet, mockPost, mockUpgradeBuilding } = vi.hoisted(() => ({
  mockGet: vi.fn(),
  mockPost: vi.fn(),
...
vi.mock('../../services/api', async () => {
  ...
  return {
    ...
    planetaryAPI: {
      ...actual.planetaryAPI,
      upgradeBuilding: (...a: unknown[]) => mockUpgradeBuilding(...a),
      getPlanet: vi.fn(),
...
describe('GameContext planetaryAPI.upgradeBuilding', () => {
  ...
  beforeEach(async () => {
    ...
    mockUpgradeBuilding.mockResolvedValue({ success: true, level: 2 });
    captured = null;
  ...
  it('upgradePlanetBuilding routes through planetaryAPI.upgradeBuilding', async () => {
    let result: unknown;
    await act(async () => {
      result = await captured!.upgradePlanetBuilding('planet-9', 'factory', 2);
      await flush();
    });
    expect(mockUpgradeBuilding).toHaveBeenCalledWith('planet-9', 'factory', 2);
    expect(result).toEqual({ success: true, level: 2 });
    const rawPosts = mockPost.mock.calls.filter((c) =>
      String(c[0]).includes('/buildings/upgrade'),
    );
