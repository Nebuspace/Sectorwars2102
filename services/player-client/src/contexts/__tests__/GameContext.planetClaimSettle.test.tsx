...
/**
 * GameContext — expeditionAPI.settle (WO-WIRE-PLANET-CLAIM-SETTLE).
 */
import React, { act } from 'react';
import { createRoot } from 'react-dom/client';
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';

const { mockGet, mockPost, mockSettle } = vi.hoisted(() => ({
  mockGet: vi.fn(),
  mockPost: vi.fn(),
  mockSettle: vi.fn(),
}));
...
describe('GameContext expeditionAPI.settle', () => {
  ...
  beforeEach(async () => {
    ...
    mockSettle.mockResolvedValue({ success: true, planet_id: 'planet-9' });
    captured = null;
  ...
  it('claimPlanet routes through expeditionAPI.settle', async () => {
    let result: unknown;
    await act(async () => {
      result = await captured!.claimPlanet('planet-9');
      await flush();
    });
    expect(mockSettle).toHaveBeenCalledWith('planet-9');
    expect(result).toEqual({ success: true, planet_id: 'planet-9' });
    const rawPosts = mockPost.mock.calls.filter((c) =>
      String(c[0]).includes('/planets/') && String(c[0]).includes('/claim'),
    );
