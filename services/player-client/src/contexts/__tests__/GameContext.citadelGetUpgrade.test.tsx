...
/**
 * GameContext — citadelAPI.getInfo / upgrade (WO-WIRE-CITADEL-GET-UPGRADE).
 */
import React, { act } from 'react';
import { createRoot } from 'react-dom/client';
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';

const { mockGet, mockPost, mockGetInfo, mockUpgrade } = vi.hoisted(() => ({
  mockGet: vi.fn(),
  mockPost: vi.fn(),
  mockGetInfo: vi.fn(),
  mockUpgrade: vi.fn(),
...
describe('GameContext citadelAPI getInfo/upgrade', () => {
  ...
  beforeEach(async () => {
    ...
    mockGetInfo.mockResolvedValue({ citadel_level: 1, citadel_name: 'Outpost' });
    mockUpgrade.mockResolvedValue({ success: true, citadel_level: 2 });
    captured = null;
  ...
  it('getCitadelInfo routes through citadelAPI.getInfo', async () => {
    let result: unknown;
    await act(async () => {
      result = await captured!.getCitadelInfo('planet-9');
      await flush();
    });
    expect(mockGetInfo).toHaveBeenCalledWith('planet-9');
    expect(result).toEqual({ citadel_level: 1, citadel_name: 'Outpost' });
    const rawGets = mockGet.mock.calls.filter((c) =>
      String(c[0]).includes('/citadel') && !String(c[0]).includes('auto-deposit'),
    );
    expect(rawGets).toHaveLength(0);
  });

  it('upgradeCitadel routes through citadelAPI.upgrade', async () => {
    let result: unknown;
    await act(async () => {
      result = await captured!.upgradeCitadel('planet-9');
      await flush();
    });
    expect(mockUpgrade).toHaveBeenCalledWith('planet-9');
    expect(result).toEqual({ success: true, citadel_level: 2 });
    const rawPosts = mockPost.mock.calls.filter((c) =>
      String(c[0]).endsWith('/citadel/upgrade'),
    );
