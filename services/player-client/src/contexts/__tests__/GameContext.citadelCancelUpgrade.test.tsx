...
/**
 * GameContext — citadelAPI.cancelUpgrade (WO-WIRE-CITADEL-CANCEL-UPGRADE).
 */
import React, { act } from 'react';
import { createRoot } from 'react-dom/client';
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';

const { mockGet, mockPost, mockCancelUpgrade } = vi.hoisted(() => ({
  mockGet: vi.fn(),
  mockPost: vi.fn(),
  mockCancelUpgrade: vi.fn(),
}));
...
vi.mock('../../services/api', async () => {
  ...
  return {
    ...
    citadelAPI: {
      ...actual.citadelAPI,
      cancelUpgrade: (...a: unknown[]) => mockCancelUpgrade(...a),
    },
    sectorAPI: {
      ...actual.sectorAPI,
...
describe('GameContext citadelAPI.cancelUpgrade', () => {
  ...
  beforeEach(async () => {
    ...
    mockCancelUpgrade.mockResolvedValue({ success: true, refunded: 500 });
    captured = null;
  ...
  it('cancelCitadelUpgrade routes through citadelAPI.cancelUpgrade', async () => {
    let result: unknown;
    await act(async () => {
      result = await captured!.cancelCitadelUpgrade('planet-9');
      await flush();
    });
    expect(mockCancelUpgrade).toHaveBeenCalledWith('planet-9');
    expect(result).toEqual({ success: true, refunded: 500 });
    const rawPosts = mockPost.mock.calls.filter((c) =>
      String(c[0]).includes('/citadel/cancel'),
    );
