...
/**
 * GameContext — planetaryAPI.getPlanet (WO-WIRE-PLANETARY-GET-PLANET).
 */
import React, { act } from 'react';
import { createRoot } from 'react-dom/client';
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';

const { mockGet, mockPost, mockGetPlanet } = vi.hoisted(() => ({
  mockGet: vi.fn(),
  mockPost: vi.fn(),
  mockGetPlanet: vi.fn(),
}));
...
vi.mock('../../services/api', async () => {
  ...
  return {
    ...
    planetaryAPI: {
      ...actual.planetaryAPI,
      getPlanet: (...a: unknown[]) => mockGetPlanet(...a),
    },
    sectorAPI: {
      ...actual.sectorAPI,
...
describe('GameContext planetaryAPI.getPlanet', () => {
  ...
  beforeEach(async () => {
    ...
    mockGetPlanet.mockResolvedValue({ id: 'planet-9', name: 'Aurora' });
    captured = null;
  ...
  it('getPlanetDetails routes through planetaryAPI.getPlanet', async () => {
    let result: unknown;
    await act(async () => {
      result = await captured!.getPlanetDetails('planet-9');
      await flush();
    });
    expect(mockGetPlanet).toHaveBeenCalledWith('planet-9');
    expect(result).toEqual({ id: 'planet-9', name: 'Aurora' });
    const rawGets = mockGet.mock.calls.filter((c) =>
      String(c[0]).match(/\/api\/v1\/planets\/planet-9$/),
    );
