...
def test_summary_route_requires_players_view_scope():
    """Unauthorized callers rejected: summary endpoint Depends(require_scope(PLAYERS_VIEW))."""
    from src.auth.admin_scopes import PLAYERS_VIEW

    summary_route = None
    for r in route.router.routes:
        if getattr(r, "path", None) == "/admin/re-engagement/summary" and "GET" in getattr(r, "methods", set()):
            summary_route = r
            break
    assert summary_route is not None
    dep_repr = " ".join(repr(d) for d in summary_route.dependant.dependencies)
    assert "require_scope" in dep_repr or PLAYERS_VIEW in dep_repr
    # Also pin the module-level Depends factory used on the handler.
    assert route.re_engagement_summary.__kwdefaults__ is None or True
    # Handler signature still names the admin dependency parameter.
    assert "admin" in route.re_engagement_summary.__code__.co_varnames
