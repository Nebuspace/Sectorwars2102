...
async def test_list_re_engagement_queue_boom_is_opaque_500():
    ...
    assert exc.detail == "Failed to fetch re-engagement queue"
...
async def test_update_re_engagement_status_boom_is_opaque_500():
    ...
    exc = excinfo.value
    assert exc.status_code == 500
    assert exc.detail == "Failed to update re-engagement status"
...
def test_admin_re_engagement_http500_is_opaque():
    ...
    assert 'detail="Failed to fetch re-engagement summary"' in src
    assert 'detail="Failed to fetch re-engagement queue"' in src
    assert 'detail="Failed to update re-engagement status"' in src
