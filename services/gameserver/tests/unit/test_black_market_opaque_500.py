...
async def test_buy_contraband_unexpected_is_opaque_500():
    ...
    assert exc.detail == {
            "error_code": "ERR_BLACK_MARKET_BUY_FAILED",
            "detail": "Black-market trade failed",
        }
...
async def test_sell_contraband_unexpected_is_opaque_500():
    ...
    assert exc.status_code == 500
    assert exc.detail == {
            "error_code": "ERR_BLACK_MARKET_SELL_FAILED",
            "detail": "Black-market trade failed",
        }
    assert secret not in str(exc.detail)
    db.rollback.assert_called_once()
...
def test_black_market_http500_catches_have_no_detail_str_e():
    ...
    assert "ERR_BLACK_MARKET_BUY_FAILED" in src
    assert "ERR_BLACK_MARKET_SELL_FAILED" in src
    assert "route_internal_error" in src
    assert 'detail="Black-market trade failed"' not in src
