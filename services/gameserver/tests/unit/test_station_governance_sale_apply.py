...
class TestApplyPassedSale:
    def test_releases_ownership_and_lists(self):
        ...
        with patch(LOCK, return_value=station), patch(LIST, return_value=listing) as mock_list:
    ...
    def test_apply_passed_sale_delegates_to_listing_helper(self):
        ...
        with patch.object(
            po, "apply_governance_sale_listing", return_value=listing
        ) as mock_apply:
...
class TestMaybeResolveRowSaleApply:
    ...
    def test_failed_sale_leaves_owner(self):
        station = _syndicate_station()
        row = SimpleNamespace(
            id=uuid4(),
            status="open",
