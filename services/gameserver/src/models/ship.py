...
class Ship(Base):
    ...
    # --- Ship Registry (ADR-0008; SYSTEMS/ship-registry.md; DATA_MODELS/ships.md)
    # Schema-only rollout (WO-P10-green-ship-registry-schema): possession
    # (current_pilot_id) split from ownership (registered_owner_id), stolen
    # status, hatch lock, and peer-to-peer listing. All ADDITIVE NULLABLE per
    # the dispatching WO -- no NOT NULL flip against existing rows in this
    # pass, and none of the report/retract/transfer/salvage/board GAMEPLAY
    # (Wave-2) lives here, only the columns + auto-registration hook below.
    #
    # Player-visible stable identifier REG-XXXX-YYYY (ship-registry.md
    # "Registration number format"). Auto-assigned by the before_insert
    # listener in src/models/ship_registry.py for every new row; NULL here
    # means a pre-registry hull that predates this migration, until a
    # follow-up backfill (ship_registry_service.backfill_initial_registrations)
    # runs.
    registration_number = Column(String(15), unique=True, nullable=True)
    # Legal owner of record (DATA_MODELS/ships.md:44 -- "formerly owner_id").
    # DOC CONFLICT NOTE: the dispatching WO's column sketch omitted this
    # field, but ship-registry.md's source map and DATA_MODELS/ships.md both
    # name it as a required Ship state addition alongside current_pilot_id/
    # stolen_status/stolen_reported_at -- added here per "follow the DOC on
    # conflict." Nullable additive column, auto-backfilled from owner_id for
    # every new row by the before_insert listener; existing rows are NOT
    # backfilled by this migration (additive-only -- a follow-up
    # backfill/data-migration is Wave-2 scope).
    registered_owner_id = Column(UUID(as_uuid=True), ForeignKey("players.id", ondelete="CASCADE"), nullable=True)
    # Whoever's currently at the controls; NULL = Drifting, ==
    # registered_owner_id = Owner aboard, anything else = Borrowed.
    # Independent of stolen_status (a Stolen ship's pilot is usually neither).
    current_pilot_id = Column(UUID(as_uuid=True), ForeignKey("players.id", ondelete="SET NULL"), nullable=True)
    ...
    # WO-FIX-SHIP-REGISTRY-BEHAVIORAL-ROUTES: report/retract-stolen writer
    # support columns (ship-registry.md "Reporting a ship stolen"). All
    # additive nullable -- no NOT NULL flip against existing rows.
    #
    # The owner's chosen recovery vehicle at filing time ("with_bounty" |
    # "no_bounty", ADR-0052 SK37). Persisted so retract can tell whether an
    # auto-bounty needs unwinding.
    stolen_recovery_mode = Column(String(16), nullable=True)
    # The current pilot at filing time -- the "thief" the auto-bounty (if
    # any) targets and the anti-collusion check (ADR-0049 SK9) keys off of.
    # Snapshotted rather than re-read from current_pilot_id at retract time
    # because the ship may change hands again before the report is retracted.
    stolen_thief_id = Column(UUID(as_uuid=True), ForeignKey("players.id", ondelete="SET NULL"), nullable=True)
    # The auto-placed BountyClaim-tracked bounty's JSONB entry id (see
    # bounty_service.place_bounty's ``bounty_entry["id"]``), so retract can
    # locate and remove the exact entry without scanning by ship linkage the
    # bounty JSONB shape doesn't otherwise carry.
    stolen_bounty_ref = Column(String(64), nullable=True)
    # Set-once flag for the 24h retract-grace expiry (ADR-0053 WR10): once
    # true, retract falls through to the standard "no refund after 24h" rule
    # even if a later sweep hasn't run. Written here directly at report-time
    # is out of scope (WR10's periodic ticker is a separate WO); retract
    # itself computes the 24h boundary from stolen_reported_at each call.
    retract_grace_processed = Column(Boolean, nullable=False, default=False, server_default=text("false"))
    # 4-8 alphanumeric hatch pin gating boarding (ship-registry.md "Hatch pin
    # lock"). NOT auto-generated by this WO -- pin issuance is Wave-2
    # boarding gameplay; the column exists so Wave-2 has somewhere to write it.
    hatch_pin_code = Column(String(8), nullable=True)
    # Delayed pin-reset (ship-registry.md "Hatch pin lock": "the registered
    # owner can always reset the pin via a port admin action (1-hour real-
    # time delay before the new pin takes effect; gives any current borrower
    # a window to extract themselves before being locked out)"). Non-NULL
    # pending_hatch_pin_effective_at means a reset is in flight; the sweep
    # (scheduler/economy_sweeps.py) applies pending_hatch_pin_code ->
    # hatch_pin_code and clears both columns once the deadline passes.
    # Mirrors the pending_transfer_* additive-columns + sweep pattern below.
    pending_hatch_pin_code = Column(String(8), nullable=True)
    pending_hatch_pin_effective_at = Column(DateTime(timezone=True), nullable=True)
    # Peer-to-peer sale listing (ship-registry.md "Trading"). Non-NULL price
    ...
    # Ship Registry relationships (DATA_MODELS/ships.md#ship "Relationships").
    registered_owner = relationship("Player", foreign_keys=[registered_owner_id])
    current_pilot = relationship("Player", foreign_keys=[current_pilot_id])
    ...
    # Append-only ownership/possession audit trail (SYSTEMS/ship-registry.md).
    # No delete-orphan cascade -- registry rows must survive ship destruction.
    registry_history = relationship("ShipRegistry", back_populates="ship", foreign_keys="ShipRegistry.ship_id")
