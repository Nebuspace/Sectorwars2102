...
APPEND-ONLY (DB-enforced): migration ``e2a7f3c8b5d1`` installs a BEFORE
DELETE/UPDATE trigger that rejects DELETE and any UPDATE that touches
columns other than ``reviewed_by`` / ``reviewed_at`` (Phase E ack path).
...
class AdminActionLog(Base):
    """One row per admin action.  Append-only (trigger + privilege revoke)."""
